...
module.exports.processMethod = async function (msg, cfg) {
  ...
  const url = transform(msg, { expression: config.url }, emitter);
  ...
  const existingAuthHeader = (headers || []).find(
    (header) => header._type === CREDS_HEADER_TYPE
  );
  ...
  async function getValidToken() {
    ...
    const oldRefreshToken = auth.oauth2.keys.refresh_token;
    auth.oauth2.keys = await fetchNewToken();
    if (!auth.oauth2.keys.refresh_token) {
      emitter.logger.debug(
        "refresh_token was not returned, use old refresh_token"
      );
      auth.oauth2.keys.refresh_token = oldRefreshToken;
    }
    ...
    if (auth.oauth2.keys.expires_in) {
      auth.oauth2.keys.tokenExpiryTime = new Date(
        tokenRefreshStartTime.getTime() + auth.oauth2.keys.expires_in * 1000
      ).toISOString();
    }
    emitter.logger.trace("Emit updateKeys = %j", auth);
  ...
  function checkErrors(response) {
    ...
    if (statusCode >= 300 && statusCode < 400) {
      if (followRedirect) {
        const REDIRECTION_ERROR = `${
          response.statusMessage || "Redirection error."
        } Please check "Follow redirect mode" if You want to use redirection in your request.`;
    ...
    if (statusCode >= 400 && statusCode < 1000) {
      ...
      let errString = `Code: ${statusCode} Message: ${
  ...
  function processResponse(response) {
    ...
    emitter.logger.trace(
      "HTTP Response body: %o",
    ...
    if (contType) {
      ...
      // if (
      //   contType.includes("image") ||
      //   contType.includes("msword") ||
      //   contType.includes("msexcel") ||
      //   contType.includes("pdf") ||
      //   contType.includes("csv") ||
      //   contType.includes("octet-stream") ||
      //   contType.includes("binary")
      // ) {
      //   return attachment.addAttachment
      //     .call(
      //       emitter,
      //       msg,
      //       `${uuidv1()}_${new Date().getTime()}`,
      //       response.body,
    ...
    emitter.logger.info(
      "Unknown content-type received. trying to parse as JSON"
    );
    return Promise.resolve(response.body)
      .then(JSON.parse)
      .then(buildResponseStructure)
